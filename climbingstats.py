...
class Event:
    ...
    def boulder_results(self):
        ...
        for sex in ["Men", "Women"]:
            ...
            for stage in ["Qualification", "Semi-final", "Final"]:
                ...
                boulders = (
                    discipline[stage]
                    .apply(lambda x: Athlete.parse_tops(x, none_to_zeros=False))
                    .dropna()
                    .to_list()
                )
                boulders = pd.DataFrame(
                    boulders,
                    columns=["tops", "zones", "top_attempts", "zone_attempts"],
                    index=discipline.name[: len(boulders)],
                )
    ...
    def from_dict(event_dict: dict):
        ...
        for category in event.categories:
            ...
            if "BOULDER " in category:
                event.results[category] = Event._parse_boulder_scores(
                    event_dict["results"][category]
                )
    ...
    def _parse_rounds(rounds: dict, round_num=3) -> list:
        ...
        starting_group = (
            rounds.loc[0, "starting_group"][-1]
            if "starting_group" in rounds.columns
            else " "
        )
    ...
    def _parse_boulder_scores(rankings: dict) -> pd.DataFrame:
        ...
        df = df[df.rounds.apply(len) > 0]
        df["name"] = (df["firstname"] + " " + df["lastname"]).str.title()

        round_names = [
            entry["round_name"].capitalize() for entry in df.loc[0, "rounds"]
        ]
        scores = pd.DataFrame(
            df.rounds.apply(
                lambda x: Event._parse_rounds(x, len(round_names))
            ).to_list(),
            columns=round_names + ["flashes", "starting_group"],
        )
        return pd.concat(
            [df[["rank", "name", "athlete_id", "country"]], scores], axis=1
        )
                    ...
                    for athlete in results.iloc:
                        ...
                        if name not in athletes:
                            athletes[name] = Athlete(
                                name, sex, athlete["country"], athlete["athlete_id"]
                            )
...
def get_event_data(filename="/data/full_data.json", period=(2008, 2023)):
...
def get_yearly_data(events, period, elite_cutoff=10):
    ...
    for year in years:
        ...
        for sex in ["Men", "Women"]:
            ...
            elite_top_perc[sex.lower()].append(
